...
import AppUsageTrackerService from './AppUsageTracker';
...
class BackgroundUpdateService {
  ...
  private async checkAndTriggerNotifications(prefs: any) {
    try {
      // Get current settings from native layer
      const appUsageTracker = AppUsageTrackerService.getInstance();
      const screenTimeLimit = await appUsageTracker.getScreenTimeLimit();
      const notificationFrequency = await appUsageTracker.getNotificationFrequency();
      
      // Check screen time limit with current settings
      await appUsageTracker.checkScreenTimeLimit({
        totalTime: prefs.totalScreenTime,
